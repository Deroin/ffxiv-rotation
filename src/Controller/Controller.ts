...
import {Buff, EnemyBuff, ResourceLocks, Resources, updateStatusDisplay} from "../Components/StatusDisplay";
import {updateSkillButtons} from "../Components/Skills";
import {updateConfigDisplay} from "../Components/PlaybackConfig"
...
class Controller {
    ...
    updateAllDisplay(game: GameState = this.game) {
        updateConfigDisplay(game.config);
    ...
    #requestTick(props: {
        ...
        if (props.deltaTime > 0) {
            let timeTicked = this.game.tick(
                props.deltaTime,
                props.prematureStopCondition ? props.prematureStopCondition : () => {
                    return false;
                });
    ...
    #useSkill(
        ...
        if (bWaitFirst) {
            ...
            if ((skillName === SkillName.Fire || skillName === SkillName.Blizzard)
                ...
                && !this.game.resources.get(ResourceType.Paradox).available(1)) {
                // and vice versa
                if (this.game.getIceStacks() > 0) {
                    skillName = SkillName.Blizzard;
                } else if (this.game.getFireStacks() > 0) {
