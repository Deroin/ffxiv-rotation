...
function PotencyDisplay(props: {
    ...
    return <span style={{textDecoration: props.includeInStats ? "none" : "line-through"}}>{potency.toFixed(2)} <Help
        topic={props.helpTopic} content={potencyExplanation}/></span>
}
...
export class DamageStatistics extends React.Component {
    ...
    render() {
        ...
        if (this.data.tinctureBuffPercentage > 0) {
            let s = lparen + (localize({
                en: "pot +",
                zh: "爆发药 +"
            }) as string) + this.data.tinctureBuffPercentage + "%" + rparen;
            potencyStr += s;
        ...
        if (this.data.totalPotency.pending > 0) {
            potencyStr += lparen + this.data.totalPotency.pending.toFixed(2) + (localize({
                en: " pending",
                zh: "未结算"
            }) as string) + rparen;
        }
        if (this.selected.potency.pending > 0) {
            selectedPotencyStr += lparen + this.selected.potency.pending.toFixed(2) + (localize({
                en: " pending",
                zh: "未结算"
            }) as string) + rparen;
        }
        ...
        let selectedGcdStr = localize({
            en: "Selected GCD skills",
            zh: "选中GCD技能"
        }) + checkedOnlyStr + colon + this.selected.gcdSkills.applied;
        if (this.data.gcdSkills.pending > 0) {
            gcdStr += lparen + "+" + this.data.gcdSkills.pending + (localize({
                en: " not yet applied",
                zh: "未结算"
            }) as string) + rparen;
        }
        if (this.selected.gcdSkills.pending > 0) {
            selectedGcdStr += lparen + "+" + this.selected.gcdSkills.pending + (localize({
                en: " not yet applied",
                zh: "未结算"
            }) as string) + rparen;
        }
        ...
        if (this.selected.duration > 0) {
            selected = <div style={{flex: 1}}>
                <div>{localize({
                    ...
                    zh: "选中时长"
                })}{colon}{this.selected.duration.toFixed(2)}</div>
        ...
        let summary = <div style={{display: "flex", marginBottom: 10, flexDirection: "row"}}>
            <div style={{flex: 1, color: this.data.historical ? colors.historical : colors.text}}>
                <div>{localize({
                    en: "Last damage application time",
                    zh: "最后伤害结算时间"
                })}{colon}{lastDamageApplicationTimeDisplay}</div>
                <div>{potencyStr}</div>
        ...
        let makeRow = function (props: {
            ...
            if (!sameAsLast && props.row.basePotency > 0) {
                includeCheckboxes.push(<input key="main" type={"checkbox"}
                                              style={{position: "relative", top: 2, marginRight: 10}}
                                              checked={includeInStats} onChange={() => {
            ...
            if (props.row.skillName === SkillName.Thunder3) {
                includeCheckboxes.push(<input key="dot" type={"checkbox"}
                                              style={{position: "relative", top: 2, marginRight: 10}} checked={
            ...
            if (!sameAsLast) {
                skillNameNode = <span>
                    <span style={{
                        ...
                        <Help topic={"potencyStats-t3"}
                              content={localize({en: "See Thunder 3 table below for details", zh: "详见下方雷3表格"})}/>
                        : undefined}
                    </span>
            ...
            let usageCountNode = <span style={{textDecoration: includeInStats ? "none" : "line-through"}}>
                {props.row.hitCount}
                {unhitUsages > 0 ?
                    <span style={{color: colors.timeline.untargetableDamageMark + "af"}}> +{unhitUsages} <Help
            ...
            if (props.row.showPotency) {
                totalPotencyNode = <span style={{textDecoration: includeInStats ? "none" : "line-through"}}>
                    ...
                    }}> +{props.row.potPotency.toFixed(2)}({localize({
                        en: "pot",
                        zh: "爆发药"
                    })})({props.row.potCount})</span> : undefined}
                </span>
            }
        ...
        let makeT3Row = function (props: {
            ...
            let numTicksNode = <span>
                ...
                {unhitTicks > 0 ?
                    <span style={{color: colors.timeline.untargetableDamageMark + "af"}}> +{unhitTicks} <Help
        ...
        let mainTable = <div style={{
            ...
            <div style={{outline: "1px solid " + colors.bgMediumContrast}}>
                <div>
                    <div style={{display: "inline-block", width: "35%"}}><span
                        style={headerCellStyle}><b>{localize({en: "skill", zh: "技能"})}</b></span></div>
                    <div style={{display: "inline-block", width: "16%"}}><span
                        style={headerCellStyle}><b>{localize({en: "potency", zh: "单次威力"})}</b></span></div>
                    <div style={{display: "inline-block", width: "14%"}}><span
                        style={headerCellStyle}><b>{localize({en: "count", zh: "数量"})}</b></span></div>
                    <div style={{display: "inline-block", width: "35%"}}><span
                        style={headerCellStyle}><b>{localize({en: "total", zh: "总威力"})}</b></span></div>
                </div>
                {tableRows}
                <div style={{
                    textAlign: "left",
                    ...
                    {this.data.mainTableTotalPotency.withoutPot.toFixed(2)}
                        {this.data.mainTableTotalPotency.potPotency > 0 ?
                            <span
                                style={{color: colors.timeline.potCover}}> +{this.data.mainTableTotalPotency.potPotency.toFixed(2)}{localize({
        ...
        let t3Table = <div style={{
            ...
            <div style={{outline: "1px solid " + colors.bgMediumContrast}}>
                <div>
                    <div style={{display: "inline-block", width: "20%"}}><span
                        style={headerCellStyle}><b>{localize({en: "time", zh: "时间"})}</b></span></div>
                    ...
                    <div style={{display: "inline-block", width: "11%"}}><span
                        style={headerCellStyle}><b>{localize({en: "initial", zh: "初始威力"})}</b></span></div>
                    <div style={{display: "inline-block", width: "11%"}}><span
                        style={headerCellStyle}><b>{localize({en: "DoT", zh: "DoT威力"})}</b></span></div>
                    <div style={{display: "inline-block", width: "10%"}}><span
                        style={headerCellStyle}><b>{localize({en: "ticks", zh: "跳雷次数"})}</b></span></div>
                    <div style={{display: "inline-block", width: "20%"}}><span
                        style={headerCellStyle}><b>{localize({en: "total", zh: "总威力"})}</b></span></div>
                </div>
                {t3TableRows}
