...
type SaveToFileProps = {
    getContentFn: () => object | Array<Array<any>>,
    filename: string,
    fileFormat: FileFormat,
    displayName?: ContentNode,
};
type SaveToFileState = {
    jsonContent: object,
    csvContent: Array<Array<any>>,
}

export class SaveToFile extends React.Component<SaveToFileProps, SaveToFileState> {
    ...
    updateContent = () => {
        ...
        if (this.props.fileFormat === FileFormat.Json) {
            this.setState({jsonContent: newContent});
        } else if (this.props.fileFormat === FileFormat.Csv) {
            this.setState({csvContent: newContent as Array<Array<any>>});
        } else {
    ...
    render() {
        let url = "";
        if (this.props.fileFormat === FileFormat.Json) {
            url = getBlobUrl(this.state.jsonContent);
        } else if (this.props.fileFormat === FileFormat.Csv) {
            url = getCsvUrl(this.state.csvContent);
        } else {
            (console.assert(false));
        }
        return <a
...
export function loadFromFile(fileObject: Blob, callback = (content: object) => {
    console.log(content)
}) {
...
export class Input extends React.Component<InputProps> {
    ...
    render() {
        ...
        return <div style={overrideStyle}>
            ...
            <input style={inputStyle} size={width} type="text" value={this.props.defaultValue}
...
export class Expandable extends React.Component<ExpandableProps, ExpandableState> {
    ...
    constructor(props: ExpandableProps) {
        super(props);
        this.props = props;
        if (props.autoIndent === false) this.autoIndent = false;
        this.onClick = (() => {
            let newShow = !this.state.show
            this.setState({show: newShow});
            if (this.props.onExpand && newShow) this.props.onExpand();
            if (this.props.onCollapse && !newShow) this.props.onCollapse();
            localStorage.setItem("exp: " + props.title, (newShow ? 1 : 0).toString());
        });

        let expanded = localStorage.getItem("exp: " + props.title);
        let show: boolean = props.defaultShow ?? false;
        if (expanded !== null) {
...
export function GlobalHelpTooltip(props: {
    ...
    return <div>
        ...
        `}</style>
        <ReactTooltip anchorSelect=".global-help-tooltip" className="help-tooltip"
                      classNameArrow="help-tooltip-arrow">{tipContent}</ReactTooltip>
    </div>
}
...
export function GlobalInfoTooltip(props: {
    ...
    return <ReactTooltip anchorSelect=".global-info-tooltip" className="info-tooltip"
