...
import {Clickable, ContentNode, Help, ProgressBar} from "./Common";
    ...
    return <div className={props.className} style={{marginBottom: 4, lineHeight: "1.5em"}}>
        ...
        <div style={{width: 200, display: "inline-block"}}>
            ...
            <div style={{
                ...
                height: "100%",
                display: "inline-block"
            }}>{props.currentStacks + "/" + props.maxStacks}</div>
        </div>
    </div>;
}
...
function Buff(props: {
    ...
    let assetName = props.rscType.toString();
    if (props.rscType === ResourceType.Triplecast) {
        if (props.stacks === 2) {
            assetName += "2";
        } else if (props.stacks === 3) {
            assetName += "3";
        }
    }
...
function BuffsDisplay(props: { data: Buff | null }) {
...
function EnemyBuffsDisplay(props: { data: EnemyBuff | null }) {
    let data = (props && props.data) ? props.data : {
        DoTCountdown: 0,
        addleCountdown: 0
    };
...
function ResourceLocksDisplay(props: { data: ResourceLocks | null }) {
    let colors = getCurrentThemeColors();
    let data = (props && props.data) ? props.data : {
        gcdReady: true,
        gcd: 2.5,
...
function ResourcesDisplay(props: { data: Resources | null }) {
    let colors = getCurrentThemeColors();
    let data = (props && props.data) ? props.data : {
        mana: 10000,
...
type StatusDisplayData = {
    ...
    resourceLocks: ResourceLocks | null,
    selfBuffs: Buff | null,
    enemyBuffs: EnemyBuff | null
}
export var updateStatusDisplay = (data: StatusDisplayData) => {
};

export class StatusDisplay extends React.Component<any, StatusDisplayData> {
    ...
    render() {
        return <div className={"statusDisplay"}>
            <div style={{position: "absolute", top: -8, right: 0, zIndex: 1}}>
                <Help topic={"mainControlRegion"}
                      content={
                          <div className="toolTip">
                              <div className="paragraph">
                                  <span style={{color: "lightgray"}}>grey</span> border:
                                  not focused
                              </div>
                              <div className="paragraph">
            ...
            <div className={"-left"}>
                <span style={{display: "block", marginBottom: 10}}>{localize({
                    en: "time: ",
                    zh: "战斗时间："
                })}{this.state.time.toFixed(2)}</span>
