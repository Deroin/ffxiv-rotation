...
function TimelineActionElement(props: {
    ...
    if (props.node.type === ActionType.Skill) {
        name = props.node.skillName ? localizeSkillName(props.node.skillName) : localize({
            en: "(unknown skill)",
            zh: "未知技能"
        });
    ...
    return <div style={style} ref={props.refObj ?? null} onClick={(e) => {
        ...
        {props.isFirstInvalid ? <span style={{
            marginRight: 6,
            padding: "0 4px",
    ...
    render() {
        ...
        let applySection = () => {
            if (this.isDirty()) {
                if (this.isValid()) {
                    return <div>
                        <div style={{backgroundColor: "rgba(255, 220, 0, 0.25)"}}>{localize({
                    ...
                    return <div>
                        <div style={{backgroundColor: "rgba(255, 0, 0, 0.25)"}}>This sequence contains invalid actions!
                            Check: {nodeName + " (" + (this.state.recordValidStatus?.invalidReason ?? "(unknown)") + ")"}</div>
                        {this.discardEditsBtn()}
                    </div>
                }
            } else {
        ...
        return <Expandable
            ...
            titleNode={<span>{localize({en: "Timeline editor ", zh: "时间轴编辑器 "})}<Help topic={"timeline editor"}
                                                                                            content={<div>
                                                                                                <div
                                                                                                    className={"paragraph"}
